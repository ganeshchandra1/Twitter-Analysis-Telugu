...
def words_per_line(doc):
    ...
    for ln in doc.split('\n'):
        ...
        for ngramLength in range(minNgramLength, maxNgramLength+1):
            ...
            for ngram in zip(*[islice(seq, i, len(terms)) for i, seq in enumerate(tee(terms, ngramLength))]):
                ngram = ' '.join(ngram)
                yield ngram
...
def predict():
    if request.method == 'POST':
        ...
        if model_name == 'NB':
            ...
            twt = sequence.pad_sequences(loaded_tokenizer.texts_to_sequences(
                [word]), maxlen=80, dtype='int32', value=0)
